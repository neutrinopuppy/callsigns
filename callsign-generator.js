...
(function (root) {
  ...
  const INTROS_FUSED = {
    instinct: {
      ...
      'Building / Tinkering': `{name} handles the jet the way they handle a stripped-down engine: methodically, no hesitation.`,
    ...
    mind: {
      ...
      'Art / Photography':    `{name} composes a frame the way they compose a sortie — every element exactly where it should be.`
    },
    edge: {
      'Sports':               `{name} plays the way they fly — full throttle, fouling out only when it's worth it.`,
      ...
      'Cooking':              `{name} cooks with the burners high and the smoke alarm off, and flies exactly that way.`,
      'Reading':              `{name} reads three chapters ahead, ignoring the bookmark, and flies past the safe altitude with the same instinct.`,
      'Building / Tinkering': `{name} welds first and documents later, and the cockpit is no different.`,
      'Art / Photography':    `{name} shoots moving, in low light, never the safe frame — and flies exactly the same.`
    },
    heart: {
      ...
      'Reading':              `{name} re-reads the parts about the people they love, and treats wingmen the same way.`,
      'Building / Tinkering': `{name} fixes things for the friends who can't fix them themselves — and flies for the wingmen who need the cover.`,
    ...
    quick: {
      ...
      'Music':                `{name} picks up a song after one verse and lands a sortie on the same kind of reflex.`,
      'Gaming':               `{name}'s reflexes run ahead of the cutscene — same in a game, same in a cockpit.`,
      'Outdoors':             `{name} hikes fast and light, talking the whole way, and flies in the same gear.`,
      'Cooking':              `{name} runs six pans, each one timed perfectly. The cockpit is just a smaller kitchen.`,
      'Reading':              `{name} reads three pages a minute and three jokes a chapter, and lands sorties just as fast.`,
  ...
  const STORIES = {
    instinct: [
      ...
      `{name} walked into debrief after a rough sortie like nothing had happened, then asked if anyone wanted to grab lunch. Word reached {homeCity} before {name} did.`
    ],
    spark: [
      ...
      `Somebody back in {homeCity} probably told {name} not to be the loudest person in the room. {name} didn't listen. The callsign caught the first time it was yelled across the ramp.`
    ],
    mind: [
      ...
      `{name} spent the whole sortie calmly narrating what the opposing flight was about to do, while it happened. By the time the jets were back on the ramp, the callsign was already on the board.`
    ],
    edge: [
      `Two people told {name} not to do it. They decided to find out anyway. It worked, the tower went silent, and {name} landed to applause they probably shouldn't have gotten.`,
    ...
    heart: [
      `When their wingman's radio cut out mid-sortie, {name} turned the whole formation around without hesitation and flew wing-to-wing all the way home. When the CO asked why, {name} just said, "That's my wingman."`,
